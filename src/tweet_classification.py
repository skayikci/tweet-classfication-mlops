...
from src.tweet_pyfunc_model import TweetClassifierModel
...
def load_data():
    ...
    try:
        ...
        print(
            f"Primary dataset not found at {data_path}, using airline sentiment data. Error: {e}"
        )
...
def create_labels(df):
    ...
    def categorize_text(text):
        ...
        if any(
            word in text_lower
            for word in ["billing", "charge", "payment", "invoice", "refund", "money"]
        ):
            return "billing"
        elif any(
            word in text_lower
            for word in ["technical", "error", "bug", "not working", "broken", "issue"]
        ):
            return "technical"
        elif any(
            word in text_lower
            for word in ["order", "delivery", "shipping", "product", "item"]
        ):
            return "orders"
        elif any(
            word in text_lower for word in ["cancel", "return", "exchange", "complaint"]
        ):
            return "complaints"
        else:
            return "general"
...
def perform_eda(df):
    ...
    plt.boxplot(
        [
            df[df["category"] == cat]["text_length"].values
...
def setup_mlflow():
    ...
    try:
        ...
        print(
            f"Experiment {experiment_name} already exists. Using existing experiment. {e}"
        )
...
class MLflowExperiment:
    ...
    def run_experiment(
        ...
        with mlflow.start_run(
            run_name=f"{model_name}_{vectorizer_name}_{datetime.now().strftime('%H%M')}"
        ):
            # Log parameters
            ...
            label_encoder_path = (
                f"models/{model_name}_{vectorizer_name}_label_encoder.pkl"
            )
            ...
            print(
                f"✅ {model_name} + {vectorizer_name}: Accuracy = {accuracy:.4f}, F1 = {f1_macro:.4f}"
            )
...
def get_model_configs():
    ...
    models = {
        ...
        "naive_bayes": {"model": MultinomialNB(alpha=1.0), "params": {"alpha": 1.0}},
        "svm": {
            "model": SVC(
                kernel="linear",
                random_state=42,
...
def hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    with mlflow.start_run(
        run_name=f"hyperparameter_tuning_{datetime.now().strftime('%H%M')}"
    ):
...
def run_model_comparison():
    ...
    tuned_model, tuned_vectorizer, best_params = hyperparameter_tuning(
...
def register_api_ready_pyfunc_model(
    final_model, final_vectorizer, le, model_name="tweet_classifier_production"
):
    """
    ...
    with mlflow.start_run(
        run_name=f"{model_name}_{datetime.now().strftime('%H%M')}"
    ) as run:
    ...
    client.transition_model_version_stage(
        name=model_name, version=latest_version, alias="production"
    )

    print(
        f"🚀 Model version {latest_version} of '{model_name}' promoted to Production."
    )
    ...
    try:
        ...
        if registered_models:
            ...
            if latest_versions:
                ...
                print(
                    f"✅ Model {model_name} version {latest_version.version} registered successfully"
                )
